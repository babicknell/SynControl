...
def run(p):
    ...
    w_slow_traj = [w_slow.copy()]
    w_tar_traj = [w_tar.copy()]
    ...
    for k in range(1, p.n_steps + 1):
        ...
        if k % int(p.save_steps/p.dt) == 0:
            ...
            w_slow_rmse.append(np.mean(
                (w_slow.flatten() - w_tar.flatten())**2)**0.5)

    y_rmse = (y_rmse/p.n_steps)**0.5
    w_slow_traj.append(np.array([wt.m for wt in weights]))
    w_tar_traj.append(w_tar.copy())
    ...
    results = {'params': p, 'W_err': w_slow_rmse, 'W_slow': w_slow_traj,
               'W_tar': w_tar_traj, 'input_rates': task.rates,
               'Y_rmse': y_rmse}
...
if __name__ == '__main__':
    ...
    if p.save:
        ...
        filename = (f'task1_{p.model}_{p.ftype}_{p.id}_eta{int(p.eta0)}_'
                    f'lag{p.lag}_lprm{lprm}_beta{p.beta}_seed{p.seed}')
